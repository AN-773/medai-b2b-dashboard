...
import { 
  Activity, Sparkles, Play, 
  Pause, RefreshCw, Terminal,
  ShieldCheck, Radio, ChevronRight, Info, Mic,
  AlertTriangle, X
} from 'lucide-react';
...
const AIAgentCenter: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden border border-white/5">
              ...
              <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-slate-400 font-bold tracking-widest text-[10px] uppercase">Fleet Command HUD</span>
                  </div>
                  <h1 className="text-4xl font-black tracking-tight">Active Agent Sena</h1>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  >
                    <RefreshCw size={24} className={isStreaming ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'} />
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-10">
                {MOCK_AGENT_STATUS.map(agent => (
                  <div key={agent.id} className="p-6 bg-white/5 rounded-3xl border border-white/5 backdrop-blur-sm group hover:bg-white/10 transition-colors">
                    <div className="flex justify-between items-start mb-4">
                       <div className={`w-3 h-3 rounded-full ${agent.status === 'online' ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.5)]' : 'bg-amber-500'}`}></div>
                       <span className="text-[10px] font-black uppercase text-slate-500">{agent.status}</span>
                    </div>
                    <p className="text-xs font-black text-white uppercase tracking-tight mb-1">{agent.name}</p>
                    <div className="flex items-center justify-between text-[10px] text-slate-400 font-bold">
                      <span>{agent.recentTasksProcessed} Tasks Sena</span>
           ...
           <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-8 space-y-4">
                 {MOCK_AI_INSIGHT_LOGS.map(log => (
                    <div key={log.id} className={`p-6 border rounded-[1.5rem] transition-all hover:shadow-md ${getSeverityStyles(log.severity)}`}>
                       <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-3">
                             <div className="p-2 bg-white/50 rounded-lg">
                                {log.severity === 'critical' ? <AlertTriangle size={18} /> : <Info size={18} />}
                             </div>
                             <div>
                                <p className="text-[10px] font-black uppercase tracking-tight opacity-60">{log.agentName}</p>
                                <p className="text-sm font-black">{log.message}</p>
                             </div>
                          </div>
                          <span className="text-[10px] font-mono font-bold opacity-40">{new Date(log.timestamp).toLocaleTimeString()}</span>
                       </div>
                       <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-black/5">
                          {log.suggestedActions.map((action, i) => (
                             <button key={i} className="px-3 py-1.5 bg-white/40 hover:bg-white text-[10px] font-black uppercase tracking-widest rounded-lg flex items-center gap-2 group transition-colors">
        ...
        <div className="w-full xl:w-96 shrink-0">
          <div className="sticky top-24 space-y-6">
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden flex flex-col min-h-[600px] max-h-[800px]">
              ...
              <div className="flex-grow p-8 overflow-y-auto custom-scrollbar bg-slate-50/50">
                <div className="prose prose-sm prose-slate">
                  {briefingText ? (
                    <div className="text-sm text-slate-700 leading-relaxed font-medium whitespace-pre-wrap animate-in fade-in duration-1000">
                      {briefingText}
                      {isStreaming && <span className="inline-block w-2 h-4 bg-indigo-400 ml-1 animate-pulse"></span>}
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center h-48 text-slate-300">
                      <Terminal size={32} className="mb-2 opacity-20" />
                      <p className="text-xs font-black uppercase tracking-widest">Handshaking with Fleet...</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-8 border-t border-slate-100 bg-white">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <p className="text-xs font-black text-slate-900 uppercase tracking-tight">Vocalize Briefing</p>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Neural Voice Engine</p>
                  </div>
                </div>
      ...
      {isLiveSessionOpen && <PsychometricianAgentSession bankDataSummary={briefingText} onClose={() => setIsLiveSessionOpen(false)} />}
    </div>
  );
};
