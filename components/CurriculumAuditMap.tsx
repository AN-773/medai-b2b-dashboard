...
import { X, Target, Zap, AlertCircle, ChevronRight, CheckCircle2, LayoutGrid, Brain } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 lg:p-12">
      ...
      <div className="relative w-full max-w-7xl bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-white/20 animate-in zoom-in-95 duration-500 flex flex-col max-h-full">
        ...
        <div className="flex-grow p-8 lg:p-12 overflow-y-auto custom-scrollbar">
            ...
            {systemsWithNames.map((sys) => (
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className={`w-3 h-3 rounded-full ${sys.status === 'Critical' ? 'bg-rose-500' : sys.status === 'Under' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                  <span className={`text-[10px] font-black uppercase tracking-widest ${sys.status === 'Critical' ? 'text-rose-600' : sys.status === 'Under' ? 'text-amber-600' : 'text-emerald-600'}`}>
                    {sys.status}
                  </span>
                </div>
                
                <h3 className="text-lg font-black text-slate-900 mb-1 truncate">{sys.name}</h3>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">{sys.count} Items Indexed</p>
          ...
          <div className="mt-12 p-10 bg-indigo-900 rounded-[3rem] text-white flex flex-col lg:flex-row items-center gap-12">
             <div className="flex-1 space-y-4">
                <div className="flex items-center gap-3">
                   <Brain size={24} className="text-indigo-400" />
                   <h2 className="text-2xl font-black uppercase tracking-tight">Institutional Cognitive Weight</h2>
                </div>
                <p className="text-indigo-200 text-sm leading-relaxed font-medium">
                  The current authoring velocity favors <strong>"Apply"</strong> level clinical reasoning. 
                  We recommend increasing <strong>"Analyze"</strong> level multi-step items in the Renal and Hematology blocks to match USMLE Step 1 difficulty benchmarks.
                </p>
                <div className="flex gap-4 pt-4">
                   <div className="text-center">
                      <div className="text-xl font-black">42%</div>
                      <div className="text-[8px] font-black uppercase tracking-widest text-indigo-400">Apply</div>
                   </div>
                   <div className="w-px h-8 bg-indigo-800 self-center"></div>
                   <div className="text-center">
                      <div className="text-xl font-black">28%</div>
                      <div className="text-[8px] font-black uppercase tracking-widest text-indigo-400">Analyze</div>
                   </div>
                   <div className="w-px h-8 bg-indigo-800 self-center"></div>
                   <div className="text-center">
                      <div className="text-xl font-black">20%</div>
                      <div className="text-[8px] font-black uppercase tracking-widest text-indigo-400">Understand</div>
                   </div>
                   <div className="w-px h-8 bg-indigo-800 self-center"></div>
                   <div className="text-center">
                      <div className="text-xl font-black">10%</div>
                      <div className="text-[8px] font-black uppercase tracking-widest text-indigo-400">Recall</div>
                   </div>
                </div>
             </div>
             <div className="shrink-0">
                <button 
                  ...
                  className="px-10 py-5 bg-white text-indigo-900 rounded-[2rem] font-black text-sm uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-4"
                >
