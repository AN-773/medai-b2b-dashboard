...
export const processCSV = (csvText: string, currentData: USMLEStandardCategory[]) => {
  const lines = csvText.split(/\r\n|\n/);
  // Deep clone to allow safe mutation
  const newData = JSON.parse(JSON.stringify(currentData)) as USMLEStandardCategory[]; 
  let importedCount = 0;
  ...
  for (let i = 1; i < lines.length; i++) {
      ...
      let systemIndex = newData.findIndex(s => s.name.toLowerCase() === systemName.toLowerCase());
      if (systemIndex === -1) {
          const newSys: USMLEStandardCategory = {
              id: `SYS-${Date.now()}-${i}`,
              name: systemName,
              page: 0,
              topics: []
